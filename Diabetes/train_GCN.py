...
class EdgeCentricRGCN(torch.nn.Module):
    def __init__(self, num_edge_types, hidden_dim=128, output_dim=1):
        super().__init__()
...
class AGPGraphDataset(Dataset):
    ...
    def get(self, idx):
        ...
        node_weight = [x/three_quarters_weight if x<three_quarters_weight else 1.0 for x in unnormalized_node_weights]
        ...
        return Data(x=x, edge_index=edge_index, edge_type=edge_type, edge_weight = edge_weight, y=torch.tensor(this_label, dtype=torch.float), node_weight=node_weight)
    ...
    for epoch in range(500):
        ...
        for batch in train_loader:
            ...
            with torch.cuda.amp.autocast(dtype=torch.float32):
                pred = model(batch)
                pred_real = pred.round()
                y_real = batch.y

                trainAccuracy = (pred_real == y_real).float().sum().item()
                trainAccuracy = trainAccuracy / y_real.size(0)
        ...
        with torch.no_grad():
            ...
            for batch in test_loader:
                ...
                correct += (pred_real == y_real).float().sum().item()
        ...
        t_acc = torch.sum(all_preds == all_trues)/len(all_preds)
...
with open('marius_split_T2Dtrue_with_int.pickle', 'rb') as f:
    T2D_dataset = pickle.load(f)

with open('marius_split_T2Dfalse_with_int.pickle', 'rb') as f:
    noT2D_dataset = pickle.load(f)
...
for i in T2D_dataset:
    ...
    for j in T2D_dataset[i]:
        ...
        GRAPHS.append(j[0])
        LABELS.append(j[1])
for i in noT2D_dataset:
    print(i)
    for j in noT2D_dataset[i]:
        print(j)
        GRAPHS.append(j[0])
        LABELS.append(j[1])
...
X_train, X_test, y_train, y_test = train_test_split(GRAPHS, LABELS, test_size=0.1, random_state=250, shuffle=True)

model = train(X_train, X_test, y_train, y_test)
checkpoint = {
    "model_state": model.state_dict()
}
torch.save(checkpoint,"./best_model_v2.pth")